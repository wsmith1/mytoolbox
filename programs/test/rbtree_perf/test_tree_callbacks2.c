...
const char *EDITION_NAME = "rbtree, callbacks edition 2";

struct my_tree_node_c2_ll_s {
	long k1;
	long k2;
	struct rb_node rb;
...
static int test_tree_callbacks2(void);

int main(void)
{
	fprintf(stdout, "Testing rbtree, %s\n", EDITION_NAME);
	test_tree_callbacks2();
	return 0;
}

static int my_tree_compar_func(long key1_k1, long key1_k2, const struct rb_node *_node2)
{
	const struct my_tree_node_c2_ll_s *node2 = rb_entry_qual(_node2, struct my_tree_node_c2_ll_s, rb, const);
	if (key1_k1 < node2->k1) {
		return -1;
	}
	else if (key1_k1 > node2->k1) {
		return 1;
	}
	else {
		if (key1_k2 < node2->k2) {
...
static int test_tree_callbacks2(void)
{
	...
	struct my_tree_node_c2_ll_s *datanodes;
	int i;
	...
	BUILD_BUG_ON(sizeof(struct my_tree_node_c2_ll_s) != TEST_RBTREE_ELEMENT_SIZE);

	random32_init_s(&rnd, 100500);
	srandom32_s(&rnd, 42);

	fprintf(stdout, "Will insert %d nodes of size %lu\n", TEST_RBTREE_NODE_COUNT,
		(unsigned long)sizeof(struct my_tree_node_c2_ll_s));
	fflush(stdout);
	...
	for (i = 0; i < TEST_RBTREE_NODE_COUNT-TEST_RBTREE_NODE_STAT_COUNT; i++) {
		int ret;
		ret = tree_perf_insert(&tree_root, datanodes[i].k1, datanodes[i].k2,
	...
	for (i = 0; i < TEST_RBTREE_NODE_COUNT-TEST_RBTREE_NODE_STAT_COUNT; i++) {
		...
		const struct my_tree_node_c2_ll_s *datanode_found;

		datanode_found_node = tree_perf_find_at(&tree_root, datanodes[i].k1, datanodes[i].k2, my_tree_compar_func);
		datanode_found = (datanode_found_node ? container_of_qual(datanode_found_node, struct my_tree_node_c2_ll_s, rb, const) : NULL);
	...
	for (i = 0; i < TEST_RBTREE_NODE_STAT_COUNT; i++) {
		...
		struct my_tree_node_c2_ll_s *datanode_found;

		datanode_found_node = tree_perf_remove_at(&tree_root, datanodes[i].k1, datanodes[i].k2, my_tree_compar_func);
		datanode_found = (datanode_found_node ? container_of(datanode_found_node, struct my_tree_node_c2_ll_s, rb) : NULL);
